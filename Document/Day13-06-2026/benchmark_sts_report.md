...
> Script: [ml-service/scripts/benchmark_sts.py](../../ml-service/scripts/benchmark_sts.py).

## 1. Vì sao chọn tác vụ STS (không phải "chấm điểm bài văn")

Hệ thống production chấm báo cáo bằng **cosine similarity** giữa embedding *đoạn báo cáo* và *tiêu chí/yêu cầu* (xem [phobert_analyzer.py](../../ml-service/models/phobert_analyzer.py)). Vì vậy thước đo trung thực nhất cho "model nào hiệu quả" là **STS – Semantic Textual Similarity**: cho 2 câu kèm **điểm tương đồng do người gán (0–5)**, đo xem **cosine của model có khớp đánh giá của người không**. Đây cũng chính là chuẩn vàng mà các paper Sentence-BERT dùng để so sánh mô hình.
...
| Mô hình | Spearman ↑ | Pearson ↑ | MAE ↓ | RMSE ↓ | cos mean | cos std |
|---|---|---|---|---|---|---|
| **vietnamese-sbert** | **0.827** | **0.839** | **0.712** | **0.913** | 0.609 | 0.261 |
| **bkai-bi-encoder** | 0.731 | 0.749 | 0.833 | 1.048 | 0.595 | 0.242 |
| **TF-IDF** (baseline) | 0.685 | 0.698 | 0.861 | 1.123 | 0.479 | 0.263 |
| **PhoBERT-CLS** (hiện tại) | **0.319** | 0.286 | 1.779 | 2.196 | 0.859 | **0.108** |

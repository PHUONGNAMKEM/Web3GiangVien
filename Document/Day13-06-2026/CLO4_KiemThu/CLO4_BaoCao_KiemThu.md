# CLO4 — Kiểm thử chức năng, tính ổn định & đánh giá hiệu quả mô hình
...
> **Cách đo:** chạy lại trên **mô hình & logic production mới nhất** của hệ thống Web3‑GiangVien — KHÔNG dùng số liệu báo cáo cũ.

---

## 1. Đánh giá bộ test cũ (`Day03-06-2026` — Postman JSON → CSV)

Bộ test PhoBERT 100 cases ở `Document/Day03-06-2026` **chưa đủ** cho yêu cầu CLO4, vì:
...
Thêm nữa, **logic chấm đã thay đổi** sau Day03: hàm `analyze()` trong [phobert_analyzer.py](../../../ml-service/models/phobert_analyzer.py) nay dùng **sliding‑window embedding** (trượt cửa sổ 254 token, mean‑pool — đọc HẾT mỗi chunk thay vì cắt 256 token). Vì vậy con số 79%/83.46% của Day03 **không còn tái lập được** trên code hiện tại → không được dùng làm số liệu nghiệm thu.

**Kết luận:** phải đo lại đầy đủ 4 chỉ số trên 3 phân hệ. Toàn bộ số dưới đây là **đo thật**, có script tái lập (mục 7).

---

## 2. Bảng tổng hợp 4 chỉ số CLO4 (đo thật 13/06/2026)

| Chỉ số | Giá trị đo được | Đối tượng / Cách đo |
|---|---|---|
| **Accuracy** | **61.0%** (như đang triển khai) · **74.0%** (ngưỡng hiệu chỉnh) | Mô hình AI PhoBERT, 100 báo cáo gán nhãn |
...
| **Throughput** | **AI: 9.9 báo cáo/phút** · **On‑chain: 9.5–18.1 tx/s** | AI đo trên CPU; on‑chain tính theo block 30M gas / 12s |

> Bổ trợ: **Precision 60.6%**, **Recall 96.6%**, **ROC‑AUC 0.697** (AI); **Deploy contract 1,386,725 gas**; **Latency AI** mean 6.1s / p95 11.6s.

---

## 3. Phân hệ AI — Accuracy, F1‑Score (mô hình `vinai/phobert-base`, logic hiện tại)

**Thiết lập:** replay y nguyên 100 báo cáo (59 MATCH / 41 MISMATCH) từ Postman collection Day03 **qua hàm `analyze()` production hiện tại** (không qua uvicorn/Postman để loại nhiễu mạng). Phân loại: feedback `"Nội dung đạt yêu cầu."` → **MATCH**, `"Cần cải thiện…"` → **MISMATCH**.

### 3.1 Confusion matrix & chỉ số

![Chỉ số AI](clo4_ai_metrics.png)

![Confusion Matrix](clo4_confusion.png)
...
### 3.2 So sánh với báo cáo Day03 & phân tích nguyên nhân
...
2. PhoBERT‑CLS có **độ phân tách cosine rất kém**: theo benchmark STS 8.628 cặp ([Day13 STS](../benchmark_sts_report.md)) `cos std` chỉ **0.108**, mean **0.859** — mọi cặp câu đều dồn quanh ~0.86, kể cả không liên quan. Spearman chỉ **0.319** (thua cả TF‑IDF 0.685).
...
### 3.3 Tính ổn định — hiệu chỉnh ngưỡng
...
→ Chỉ cần **đổi quy tắc phân loại sang ngưỡng điểm ~6.3** (không đổi model) đã nâng Accuracy lên **74%**, F1 **81.2%**. Đây là bằng chứng về **tính ổn định có thể khôi phục bằng hiệu chỉnh**; trần phân biệt của model (AUC 0.697) vẫn hạn chế do bản chất PhoBERT‑CLS.
...
> Hệ thống chạy testnet **Sepolia** (ETH test miễn phí) → chi phí thực tế khi vận hành = 0đ; bảng trên quy đổi mang tính minh hoạ chi phí tương đương nếu lên mainnet. Contract V2 đã tối ưu (`bytes32` key thay `string`) nên gas thấp.

---

## 5. Throughput

![Throughput](clo4_throughput.png)

**5.1 AI (chấm báo cáo).** 100 báo cáo trong **609s** trên CPU → **9.9 báo cáo/phút** (0.164 rps). Latency: mean **6,090ms**, p50 5,351ms, p95 **11,561ms**, max 18,465ms. Đây là đặc thù PhoBERT sliding‑window đọc hết PDF trên CPU; có GPU hoặc đổi sang SBERT sẽ nhanh hơn nhiều lần.

**5.2 On‑chain (ghi blockchain).** Tính theo block Ethereum/Sepolia (30,000,000 gas/block, 12s/block) → mỗi giao dịch:
...
- **Mô hình AI cần cải thiện phân loại:** logic sliding‑window đọc đủ nội dung nhưng PhoBERT‑CLS phân tách cosine kém → Accuracy thực tế (61%) thấp hơn báo cáo cũ. **Hai hướng khắc phục (đã có bằng chứng trong repo):**
  1. **Ngắn hạn (không đổi model):** đổi quy tắc phân loại sang **ngưỡng điểm ~6.3** → Accuracy 74%, F1 81.2%.
  2. **Dài hạn (khuyến nghị chính):** đổi model luồng chấm sang **`keepitreal/vietnamese-sbert`** (Spearman 0.827 vs 0.319 của PhoBERT — xem [Day13 STS](../benchmark_sts_report.md), [Day11](../../Day11-06-2026/benchmark_report.md)), bỏ hệ số ×1.5. Vừa tăng độ chính xác vừa tăng throughput.

---

## 7. Cách tái lập (scripts)
...
| [benchmark_clo4_charts.py](../../../ml-service/scripts/benchmark_clo4_charts.py) | Sinh 4 biểu đồ PNG | `cd ml-service && python scripts/benchmark_clo4_charts.py` |

**File kết quả** (thư mục này):
`CLO4_AI_PhoBERT_Results.csv` · `CLO4_AI_metrics.json` · `CLO4_Gas_Results.csv` · `CLO4_Gas_metrics.json` · 4 biểu đồ `clo4_*.png`.

> **Giả định/giới hạn:** (1) AI đo trên CPU (torch cpu) — GPU sẽ cho throughput cao hơn. (2) Quy đổi tiền tệ theo ETH=3,000 USD, USD=25,400 VND (chỉnh trong đầu `benchmark_clo4_gas.js`). (3) Throughput on‑chain tính theo tham số block Ethereum/Sepolia chuẩn. (4) Gas là số **xác định** (không đổi giữa các lần chạy); Accuracy/F1 cố định vì model ở chế độ `eval()` deterministic.
